...
?>
      <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4" id="main">
        ...
        <div class="table-responsive view" id="show_data">
          <table class="table table-striped table-sm" id="list-mtt">
            <thead>
              <tr>
                <th>Titullo</th>
                <th>Subtitulo</th>
    ...
    function consultar(){
      ...
      $.post("includes/_funciones.php", obj, function(respuesta){
        ...
        $.each(respuesta,function(i,e){
          ...
          <tr>
          <td>${e.titulo_mtt}</td>
          <td>${e.subtitulo_mtt}</td>
          <td>${e.img_mtt}</td>
          <td>${e.titulo_img_mtt}</td>

          <td>
          <a href="#" data-id="${e.id_mtt}" class="editar_registro_mtt">Editar</a>
          <a href="#" data-id="${e.id_mtt}" class="eliminar_registro_mtt">Eliminar</a>
          </td>
          </tr>
          `;
        });
        $("#list-mtt tbody").html(template);
      },"JSON");
    }
...
//eliminar usuarios
    $("#main").on("click",".eliminar_registro_mtt" , function(e){
      e.preventDefault();
      let confirmacion= confirm("Desea eliminar este registro");
      if (confirmacion) {
        let id=$(this).data('id'),
            obj ={
              "accion":"eliminar_registro_mtt",
              "id":id
            };
            $.post("includes/_funciones.php", obj, function(respuesta){
              alert(respuesta);
...
//editar registro
$('#list-mtt').on("click",".editar_registro_mtt", function(e){
        e.preventDefault();
        let id = $(this).data('id'),
            obj = {
              "accion" : "editar_registro_mtt",
              "id" : id
            };
        $("#form_data")[0].reset();
        change_view('insert_data');
        ...
        $.post("includes/_funciones.php", obj, function(r){
          $("#titulo").val(r.titulo_mtt);
          $("#subtitulo").val(r.subtitulo_mtt);
          $("#img").val(r.img_mtt);
          $("#img_titulo").val(r.titulo_img_mtt);
